...
public class KafkaUtilTest implements AutoCloseable {
    ...
    public static void produceRandomTextData() {
        Runnable generateTask = () -> {
            ...
            int counter = 0;
            while (counter++ < 5) {
                List<String> textValues = getRandomText();

                for (String value : textValues) {
                    ProducerRecord<String, String> record = new ProducerRecord<>("topic-test",
                                                                                 null,
                                                                                 value);
            ...
            logger.info("Done generating text data");
        };
        executorService.submit(generateTask);
    }

    private static List<String> getRandomText() {
