...
public abstract class AbstractRemoteEntryPoint implements RemoteWorkingMemory {
    ...
    public <T> CompletableFuture<Collection<T>> getObjects(Class<T> clazztype) {
        ListObjectsCommand command = new ListObjectsCommandClassType(entryPoint,
                                                                     clazztype);
        return executeCommand(command);
    }
    ...
    public CompletableFuture<Collection> getObjects(String namedQuery,
                                                    String objectName,
                                                    Object... params) {
        ListObjectsCommand command = new ListObjectsCommandNamedQuery(entryPoint,
                                                                      namedQuery,
                                                                      objectName,
                                                                      params);
        return executeCommand(command);
    ...
    protected <T> CompletableFuture<T> executeCommand(AbstractCommand command) {
        ...
        getRequestsStore().put(command.getId(),
                               callback);
